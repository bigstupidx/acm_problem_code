...
void move_one_step(){
	...
	if(city[1][1]){
		...
		tim/60, name[dir[1][k%5]], k+1, nod[1][k].life, nod[1][k].power);
	}
	for(int i=1;i<=n;i++){
		city[1][i]=city[1][i+1];
	}
	...
	for(int i=1;i<=n;i++){
		if(city[0][i]){
			...
			tim/60, name[dir[0][k%5]], k+1, i, nod[0][k].life, nod[0][k].power);
		}
		if(city[1][i]){
			k=city[1][i]-1;
			printf("%.3d:10 blue %s %d marched to city %d with %d elements and force %d\n",
			tim/60, name[dir[1][k%5]], k+1, i, nod[1][k].life, nod[1][k].power);
		}
	}
	if(idd!=-1){
		city[0][n+1]++;
		printf("%.3d:10 red %s %d reached blue headquarter with %d elements and force %d\n",
		tim/60, name[dir[0][idd%5]], idd+1, nod[0][idd].life, nod[0][idd].power);
	}
	if(city[1][0]>=2){
		xflag=1;
...
void get_value(){
	...
	for(int i=1;i<=n;i++){
		if(city[0][i]&&!city[1][i]){
			...
			tim/60, name[dir[0][k%5]], k+1, val[i]);
			sum[0]+=val[i];
			val[i]=0;
		}
			...
			tim/60, name[dir[1][k%5]], k+1, val[i]);
			sum[1]+=val[i];
			val[i]=0;
		}
	}
	...
	for(int i=1;i<=n;i++){
		if(city[0][i]&&city[1][i]){
			...
			if(flag[i]==1||flag[i]==0&&i%2==1){
				...
				tim/60, name[dir[0][k1%5]], k1+1, name[dir[1][k2%5]], k2+1, i, nod[0][k1].life, nod[0][k1].power);
				tem=nod[1][k2].life;
				nod[1][k2].life-=nod[0][k1].power;
				if(nod[1][k2].life<=0){
					city[1][i]=0;
					printf("%.3d:40 blue %s %d was killed in city %d\n", tim/60, name[dir[1][k2%5]], k2+1, i);
					if(k1%5==2){
					...
					if(k2%5!=2){
						...
						tim/60, name[dir[1][k2%5]], k2+1, name[dir[0][k1%5]], k1+1, i);
						tem=nod[0][k1].life;
						nod[0][k1].life-=nod[1][k2].power/2;
						if(nod[0][k1].life<=0){
							city[0][i]=0;
							printf("%.3d:40 red %s %d was killed in city %d\n", tim/60, name[dir[0][k1%5]], k1+1, i);
							if(k1%5==1){
								nod[1][k2].life+=tem;
							}
							if(sum[1]>=8){
				...
				tim/60, name[dir[1][k2%5]], k2+1, name[dir[0][k1%5]], k1+1, i, nod[1][k2].life, nod[1][k2].power);
				tem=nod[0][k1].life;
				...
				if(nod[0][k1].life<=0){
					...
					if(k1%5!=3){
						...
						tim/60, name[dir[0][k1%5]], k1+1, name[dir[1][k2%5]], k2+1, i);
						tem=nod[1][k2].life;
