        ...
        while(qq--){
            ...
            if(u==0&&v==0)
            {
                for(int j=1;j<=row;j++)
                    ...
                    {
                        if(l[j][k]<=va && va<=c1[j][k])
                        {
                            if(ch[0]=='=')
                                l[j][k]=c1[j][k]=va;
                            else if(ch[0]=='>'){
                                if(va+1<=c1[j][k])l[j][k]=Max(va+1,l[j][k]);
                                else flag=1;
                            }
                            else{
                                if(va-1>=l[j][k])c1[j][k]=Min(va-1,c1[j][k]);
                                else flag=1;
                            }
                        }
                        else flag=1;
                ...
                for(int j=1;j<=row;j++)
                {
                    if(l[j][row+v]<=va&&va<=c1[j][row+v])
                    {
                        if(ch[0]=='=')c1[j][row+v]=l[j][row+v]=va;
                            ...
                            if(va+1<=c1[j][row+v])l[j][row+v]=Max(va+1,l[j][row+v]);
                            else flag=1;
                        }
                        else{
                            if(va-1>=l[j][row+v])c1[j][row+v]=Min(va-1,c1[j][row+v]);
                            else flag=1;
                        }
                    }
                    else {flag=1;break;}
            ...
            else if(v==0)
            {
                for(int j=row;j<=row+col;j++)
                    if(c1[u][j]>=va&&l[u][j]<=va)
                    {
                        if(ch[0]=='=')c1[u][j]=l[u][j]=va;
                            ...
                            if(va+1<=c1[u][j])l[u][j]=Max(va+1,l[u][j]);
                            else flag=1;
                        }
                        else{
                            if(va-1>=l[u][j])c1[u][j]=Min(va-1,c1[u][j]);
                            else flag=1;
                        }
                    }
                    else flag=1;
                ...
                if(l[u][row+v]<=va&&c1[u][row+v]>=va)
                {
                    if(ch[0]=='=')l[u][row+v]=c1[u][row+v]=va;
                        ...
                        if(va+1<=c1[u][row+v])l[u][row+v]=Max(va+1,l[u][row+v]);
                        else flag=1;
                    }
                    else {
                        if(va-1>=l[u][row+v])c1[u][row+v]=Min(va-1,c1[u][row+v]);
                        else flag=1;
                    }
                }
                else flag=1;
