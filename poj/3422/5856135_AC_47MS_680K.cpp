...
int head[6000],dis[6000],n,v[6000],pre[6000],map[60][60];
const int inf=1000000000;

void Add_Edge(int u,int v,int c,int w,int &top)
{
    e[++top].u=u;   e[top].v=v;     e[top].c=c;
    e[top].w=w;     e[top].next=head[u];   head[u]=top;

    e[++top].u=v;   e[top].v=u;     e[top].c=0;
    e[top].w=-w;    e[top].next=head[v];    head[v]=top;
}
bool Spfa(int s)
{
    queue<int>Q;
    for(int i=0;i<=n;i++){v[i]=0;dis[i]=inf;}
