...
const double inf = 1000, eps = 1e-18;
	...
	while (1) {
		...
		while (top >= 0) {
			if (p[top][0] != t) {
				while (p[top][1] >= 0) {
					int j = p[top][1];
					if (L[e[j].to] > L[p[top][0]] && e[j].c - e[j].f > eps) {
						p[top + 1][0] = e[j].to;
...
int main() {
	...
	while (scanf("%d%d", &n, &m) != EOF) {
		...
		double lf = 0, rt = m, mid, tem;
		while (lf < rt) {
			mid = (lf + rt) / 2;
			for (int i = 0; i < top; i++)e[i].f = 0;
			for (int i = tot; i < top; i += 2)e[i].c = mid;
			tem = max_flow(s, t);
			//printf("try: %lf %lf\n", mid, tem);
			if (tem >= m) rt = mid - 1e-6;
			else lf = mid + 1e-6;
		}
		if (lf < 1 + eps) {
			printf("1\n1\n");
		...
		//printf("%lf\n", lf);
		for (int i = 0; i < top; i++)e[i].f = 0;
		for (int i = tot; i < top; i += 2)e[i].c = lf;
		max_flow(s, t);

		vector <int> ans;
		for (int j = head[t]; j != -1; j = e[j].next) {
			//printf("%d %lf %lf\n", e[j].to, e[j^1].c, e[j^1].f);
			if (fabs(e[j^1].c - e[j^1].f) < 1e-5) {
