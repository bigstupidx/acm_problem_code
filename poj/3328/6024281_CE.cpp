...
struct edge{
    ...
    int d;
    edge *next;
}edg[100010],*vert[2][2000];
int n,m,dis[2][2000],mark[2][2000],map[61][31];
...
void Dijkstra()
{
    ...
    while(!Q.empty()){
        now=Q.top();Q.pop();
        int i=now.ss^1,k=now.u;
        if(mark[i^1][k])continue;
        mark[i^1][k]=1;
      //  printf("%d [%d][%d]->",i^1,k/m+1,k%m+1);
        for(edge *p=vert[i^1][k];p;p=p->next)
        {
            if(!mark[i][p->to]&&dis[i][p->to]>dis[i^1][k]+p->d)
            {
                dis[i][p->to]=dis[i^1][k]+p->d;
                now.u=p->to;
                now.d=dis[i][p->to];
                now.ss=i;
             //   printf(" [%d][%d] %d",p->to/m+1,p->to%m+1,dis[i][p->to]);
                Q.push(now);
            }
        }
       // puts("");
...
int main()
{
    while(scanf("%d%d",&m,&n),n||m)
    {
        ...
        for(int i=0;i<n;i++){
            for(int j=0;j<m;j++){
                if(map[i][j]==inf){
                    //printf("s: [%d][%d] %d %d\n",i+1,j+1,dis[0][i*m+j],dis[1][i*m+j]);
