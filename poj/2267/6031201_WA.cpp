...
int n,m,dis[120],mark[120],gra[120][120],edg[1010][3];
...
int main()
{
    ...
    for(int ii=1;ii<=tt;ii++)
    {
        ...
        for(int i=0;i<m;i++)
        {
            ...
            if((st+tim)%24>6&&(st+tim)%24<18)continue;
            p=mp.find(aa);
            if(p==mp.end())
            {
                mp[aa]=n;
            ...
            gra[u][v]=0;
            edg[k][0]=u;edg[k][1]=v;edg[k++][2]=st;
        }
        for(int i=0;i<k;i++)
        {
            for(int j=0;j<k;j++)
            {
                if(edg[i][1]==edg[j][0])
                {
                    if(edg[i][2]<edg[j][2]&&edg[j][2]<7)
                        gra[edg[i][1]][edg[j][0]]=0;
                    else if(edg[i][2]<edg[j][2]&&edg[i][2]>7)
                        gra[edg[i][1]][edg[j][0]]=0;
                    else if(edg[j][2]<7&&edg[i][2]>7)
                        gra[edg[i][1]][edg[j][0]]=0;
                    else
        ...
        else printf("Vladimir needs 2 litre(s) of blood.\n",dis[v]);
    }
    return 0;
}
