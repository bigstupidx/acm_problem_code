...
void solve(int c,int s,int e)
{
    ...
    while(!Q.empty())
    {
        ...
        for(edge *p = vert[now.u]; p ;p = p->next)
        {
            next.u=p->v;
            if(p->dis <= now.mass)
            {//ÔÚ´ËÕ¾²»¼ÓÓÍ£¬ÐèÒªµ±Ç°µÄÓÍÁ¿×ã¹»µ½´ïÏÂÒ»Õ¾ 
                next.mass = now.mass - p->dis;
                next.total=now.total;
                Q.push(next);
            }
            next.total=now.total+price[now.u];
            if(now.mass+1 >= p->dis)
                next.mass = now.mass + 1 - p->dis;
            else
            {
                next.mass = now.mass + 1;
                next.u=now.u;
            }
            Q.push(next);
        }
